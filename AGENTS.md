...
- `RequestLoggingMiddleware` and `OriginValidationMiddleware` are part of the backend request flow for correlation IDs and allowed-origin enforcement.
...
- Coverage reports are generated by the package scripts, but the repo does not currently enforce a global coverage gate. Prefer meaningful, behavior-focused tests over coverage-only churn.
...
- Do not modify production code when the task is limited to documentation or test updates; for docs-only or test-only work, keep runtime behavior unchanged and validate the relevant files or scripts instead.
