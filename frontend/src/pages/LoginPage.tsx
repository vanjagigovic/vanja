...
function getReturnLocation(state: unknown) {
...
export function LoginPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate(returnLocation ?? getDefaultCalendarPath(), { replace: true });
    } catch (submitError) {
      setError(getErrorMessage(submitError, t("authLoginError")));
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <CardContent sx={{ p: { xs: 3, sm: 4 } }}>
          <Stack component="form" spacing={2.5} onSubmit={handleSubmit}>
            ...
            <Button type="submit" variant="contained" disabled={submitting}>
              {submitting ? <CircularProgress size={22} color="inherit" /> : t("loginSubmit")}
            </Button>
